...
//        nextMonthButton.layer.cornerRadius = CGFloat(buttonRadius)
    ...
    func generateCalendar() {
        ...
        //Game判定
        let gameObject = GameScheduleLogic()
        var gameTag:Int = 0
        ...
        for i in 0...41{
            ...
            // button.setBackgroundImage(UIImage(named:"ms"),forState:.Normal)
            button.frame = CGRect(
                x: CGFloat(positionX),
                y: CGFloat(positionY),
            ...
            if (i < dayOfWeek - 1) {
               ...
               // button.contentEdgeInsets = UIEdgeInsetsMake(20, 20, 0, 0)
                ...
                gameTag = gameObject.judgeGameDay(year, month: month, day: tagNumber)
            ...
            //@remark:このサンプルでは正円のボタンを作っていますが、背景画像の設定等も可能です。
                        if (i % 7 == 0 || holidayFlag == true) {
                            ...
                            calendarFontColor = UIColor.lightGray
                        }
            calendarBackGroundColor = UIColor(red: 0.0, green: 0.0, blue: 0.0, alpha: 0.0)
            if (gameTag == 1) {
                calendarImage = UIImage(named:"fighters")
            } else if (gameTag == 2) {
                calendarImage = UIImage(named: "hawks")
            } else if(gameTag == 3){
                calendarImage = UIImage(named: "eagles")
            }else if(gameTag == 4){
                calendarImage = UIImage(named:"lions")
            }else if(gameTag == 5){
                calendarImage = UIImage(named: "buffaloes")
            }else if(gameTag == 0)
            {
               calendarImage = UIImage(named: "")
            }
            
            //ボタンのデザインを決定する
            button.backgroundColor = calendarBackGroundColor
            button.setBackgroundImage(calendarImage,for:UIControlState())
            button.setTitleColor(calendarFontColor, for: UIControlState())
            button.titleLabel!.font = UIFont(name: "Helvetica-Bold",size: CGFloat(calendarFontSize))
    ...
    func setupCalendarTitleLabel() {
        calendarBar.text = String("\(year)年\(month)月の日程")
    }
    ...
    func buttonTapped(_ button: UIButton) {
        ...
        //コンソール表示
       // showLabel.text = button.titleLabel?.text
